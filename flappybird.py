...
class Bird(pygame.sprite.Sprite):
	...
	def update(self,delta_frame = 1):
		if self.msec_to_climb >0:
			...
			# print(self.msec_to_climb,frac_climb_done)
	...
	@property
	def mask(self):
		if pygame.time.get_tick()%500>=250:
...
class PipePair(pygame.sprite.Sprite):
	...
	def __init__(self,pipe_end_img,pipe_body_img):
		...
		total_pieces_end_x = self.top_height_px
		self.image.blit(pipe_end_img,(0,total_pieces_end_x))
		self.top_pieces +=1
	...
	while not done:
		...
		for event in pygame.event.get():
			if event.type == QUIT or (event.type == KEYUP and event.type == K_ESCAPE):
				...
				paused = not paused
			elif event.type == MOUSEBUTTONUP or(event.type == KEYUP or event.type in [K_UP,K_RETURN,K_SPACE]):
		...
		for x in (0,win_WIDTH/2,win_WIDTH/2):
			win.blit(images['bg'],(x,0))
		...
		win.blit(score_surface,(score_x,PipePair.PIECE_HEIGHT))
